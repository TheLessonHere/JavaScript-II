...
// Problem 1 The event director needs the emails of all the runners so that he can tell them that the event time changed. Create a new array runnerEmails that contains these values.
let runnerEmails = runners.map((runner) => {
    return runner.email;
});
console.log(runnerEmails);

// Problem 2 The event director needs to know how many runners need size S shirts. Log the amount of people who need S shirts.
let smallShirts = runners.filter((runner) => {
    return runner.shirt_size === 'S';
});
console.log(smallShirts.length);

// Problem 3 The event director wants to know how much was donated by runners with XL shirts. Create a new array xlShirts and log the total.
let xlShirts = runners.filter((runner) => {
    return runner.shirt_size === 'XL';
});
let xlDonationTotal = xlShirts.reduce((total, runner) => {
    return total += runner.donation;
}, 0);
console.log(xlDonationTotal);